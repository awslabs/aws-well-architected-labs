...
1. Observe that ELB 5XXs (Count) errors corresponds to the same HTTP 502s (Count) errors
      ...
      ![AllUnhealthy502](Images/InsertImageHere.png)
...
* From the **Target Groups** console click on the  the **Health checks** tab
      * The ELB health check is configured to return _healthy_ when it received a http 200 response on the same port and path as our browser requests
...
1. For **Path** enter **/healthcheck**
1. Click **Save**
1. Click the **Targets** tab so you can use this to monitor health check status
...
1. Go to the **Target Groups** console click on the **Targets** tab and note the health status as per the ELB health checks
...
1. From the **Target Groups** console **Targets** tab note that the server in us-east-2c is marked as unhealthy
...
1. Confirm the service is health
      * Refresh the web service multiple times and note that personalized recommendations are n being served from all three servers
...
* Then you implemented error handling and the following were observed
      * The service now returned a static response (as per the error handling code)
...
1. Refresh the health check URL multiple times
      * The deep health check fails for all servers, since the  **RecommendationServiceEnabled** is not available
1. From the **Target Groups** console **Targets** tab note the health check status of all the servers (you may need ot refresh)
      * They all report _unhealthy_ with http code 503 - the code the deep health check is configured to return
